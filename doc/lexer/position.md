...
It abstracts raw byte offsets into human-readable coordinates
to track coordinates of tokens and errors.

`Position` struct contains:

- `offset`  - zero-based byte position from the start of the source text.
    It is used for internal indexing and always corresponds
